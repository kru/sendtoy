    ...
    if (g_ctx.io_req_type == IO_READ_CHUNK) {
        ...
        if (fname) {
            ...
            if (f) {
                ...
                ev.tcp.socket = (u64)g_ctx.io_req_job_id; // Pass Job ID via socket field
                ev.tcp.success = true;
                state_update(&g_ctx, &ev, platform_get_time_ms());
                // We might need to handle the NEW IO request generated by state_update immediately?
                // Yes, because loop waits for next event.
                // Let's recurse or loop? Recursing is dangerous.
                // But handle_io_request is called from loop.
                // If we call state_update here, it might set io_req_type.
                // We should process it?
                // Yes. But we are inside handle_io_request.
                // It's safer to not recurse handle_io_request but to handle it if we can.
                // Actually, just let the next tick handle it? No, next tick is 100ms away or network event.
                // We want immediate turn-around.
                // Let's call a helper or just loop in main?
                // Calling state_update sets g_ctx.io_req_type.
                // If we return, main loop won't see it until next event?
                // Main loop:
                //  state_update()
                //  handle_io_request()
                //  poll()
                // If we update state here, io_req_type is set.
                // But we are at end of handle_io_request.
                // If we return, we go to poll(). poll() waits.
                // So we STALL until next packet or tick!
                // FIX: We must handle the new IO request here or force a loop.
                // Let's handle it recursively once (tail recursion optimization manual).
                // Or better: Change main loop to "while io_req_type != NONE".
                // But for now, let's just call handle_io_request(sock) recursively? failure stack depth?
                // It's just one level (Write -> Send REQ -> None).
                // So it's safe-ish.
